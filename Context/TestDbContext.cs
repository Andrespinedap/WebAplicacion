...
namespace WebAplicacion.Context
{
    public class TestDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            var Comentari_Client = modelBuilder.Entity<Comentari_Client>();

            // Nombre de la tabla / propiedades de los campos
            Comentari_Client.ToTable("ComentariosClientes");
            Comentari_Client.HasKey(c => c.Id);
            Comentari_Client.Property(c => c.Order_Id).HasColumnType("varchar(16)");
            Comentari_Client.Property(c => c.Client_Id).HasColumnType("varchar(16)");
            Comentari_Client.Property(c => c.Comment).HasColumnType("varchar(256)");
            Comentari_Client.Property(c => c.Qualification).HasColumnType("varchar(256)");

            //Mapeo de relaciones relacion de uno a uno
            Comentari_Client.HasOne(c => c.Maintenance_History).WithOne(c => c.Comentari_Client).HasForeignKey<Comentari_Client>(c => c.Maintenance_History).OnDelete(DeleteBehavior.NoAction);
            Comentari_Client.HasOne(c => c.Client).WithOne(c => c.Comentari).HasForeignKey<Comentari_Client>(c => c.Client_Id).OnDelete(DeleteBehavior.NoAction);
            ...
            order.HasKey(x => x.Id);
            order.Property(x => x.Date).HasColumnName("Datetime").IsRequired();
            ...
            order.HasOne(x => x.Services_Orders).WithOne(x => x.Order).HasForeignKey<Services_Orders>(x => x.Oreder_Id).OnDelete(DeleteBehavior.NoAction);
            order.HasOne(x => x.Maintenance_History).WithOne(x => x.Order).HasForeignKey<Maintenance_History>(x => x.Vehicle_Id).OnDelete(DeleteBehavior.NoAction);
            ...
            order.HasOne(x => x.Inventory_Orders).WithOne(x => x.Order).HasForeignKey<Inventory_Orders>(x => x.Order_Id).OnDelete(DeleteBehavior.NoAction);
            ...
            client.Property(x => x.Address).HasColumnType("varchar(128)").IsRequired();
            client.Property(x => x.Phone).HasColumnType("varchar(16)").IsRequired();
            ...
            servicesOrders.Property(x => x.Oreder_Id).HasColumnType("varchar(8)").IsRequired();
            ...
            //Mapeo de relaciones
            MaintenanceHistory.HasOne(x => x.Vehicle).WithOne(x => x.Maintenance_History).HasForeignKey<Maintenance_History>(x => x.Vehicle_Id).OnDelete(DeleteBehavior.NoAction);
            MaintenanceHistory.HasOne(x => x.Order).WithOne(x => x.Maintenance_History).HasForeignKey<Maintenance_History>(x => x.Id).OnDelete(DeleteBehavior.NoAction);
            ...
            Dates.Property(x => x.Motivated).HasColumnType("varchar(256)").IsRequired();
            Dates.Property(x => x.Data_Time).HasColumnType("Datetime").IsRequired();
